...
#include <string.h>
// sys/ioctl.h - input/output control
#include <sys/ioctl.h>
// sys/types.h - system types
#include <sys/types.h>
// termios.h - terminal input/output
#include <termios.h>
...
void editorSelectSyntaxHighlight() {
  ...
  // iterate through HLDB
  for (unsigned int j = 0; j < HLDB_ENTRIES; j++) {
    // get syntax
    struct editorSyntax *s = &HLDB[j];
    // iterate through filematch
    unsigned int i = 0;
    while (s->filematch[i]) {
...
int editorRowCxToRx(erow *row, int cx) {
  ...
  int j;
  for (j = 0; j < cx; j++) {
    // if tab, increment rx by tab stop minus remainder of rx divided by tab
    // stop
    if (row->chars[j] == '\t') {
...
int editorRowRxToCx(erow *row, int rx) {
  ...
  for (cx = 0; cx < row->size; cx++) {
    ...
    // if rx is greater than or equal to rx, return cx
    if (cur_rx > rx) {
      return cx;
    }
  }
...
void editorUpdateRow(erow *row) {
  ...
  for (j = 0; j < row->size; j++) {
    //  if tab, increment tabs
    if (row->chars[j] == '\t') {
      tabs++;
    }
  }
  // free render and allocate memory for render
  free(row->render);
  ...
  // iterate through row
  for (j = 0; j < row->size; j++) {
    // if tab, set render to space and increment idx until idx is divisible by
    // tab stop
    if (row->chars[j] == '\t') {
      row->render[idx++] = ' ';
...
void editorInsertRow(int at, char *s, size_t len) {
  // if at is less than 0 or greater than number of rows, return
  if (at < 0 || at > E.numrows) {
    return;
  }

  // realloc memory for rows
  E.row = realloc(E.row, sizeof(erow) * (E.numrows + 1));
  // memmove - copies len bytes from string s to string E.row
